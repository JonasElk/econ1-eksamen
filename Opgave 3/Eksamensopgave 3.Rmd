...
Uddannelse vil være endogen, hvis den er korreleret med fejlledet, hvilket skaber en bias. Dette vil være fordi der er en relevant variabel, som er udeladt og derfor indgår i fejlledet. Denne udeladte variabel kunne eksempelvis være "ability", som påvirker uddannelsesniveauet positivt og dermed skaber en bias. Ligeledes kan forældrenes uddannelsesniveau eller antallet af søskende være korreleret med uddannelsesniveauet.
...
Two stage least square er en metode der anvendes, når OLS giver inkonsistente og skæve estimater. Metoden er næsten det samme som IV, men 2SLS bruges ved mere end én IV til en variabel, hvorfor "fitted values" bruges som IV. 
For hvert IV laves en hypotesetest hvorvidt de er lig 0, for derefter at estimere den reducerede ligning ved OLS, hvor det er gældende at alle estimaterne og residualerne  $v$ har en covariance på 0. 
$$y_2=\pi_0+\pi_1x_1+\pi_2z_1+\pi_3z_2+v$$
...
Ovenstående test har til formål at belyse hvorvidt instrumentvariablene er relevante for den endogene variable i den oprindelige model. Der er her tale om en f-test. Nulhypotesen afvises grundet den lave p-værdi, og dermed kan det konkluderes, at mindst en af instrumentvariablene er signifikant, og dermed relevant for modellen. 
...
Ud fra ovenstående værdi, kan det konluderes, at nulhypotesen kan afvises, og derfor at modellen er overidenticieret.
...
summary(endo_model)
```
Ud fra ovenstående model, kan det ses at residualerne er signifikante, hvilket nødvendigvis må betyde at der er endgonitet. 
...
Grundet den meget lave p-værdi, kan nulhypotesen forkastes. 
