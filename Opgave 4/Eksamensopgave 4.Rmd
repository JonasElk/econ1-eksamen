...
For at estimere modellen bruges der her brug af LPM, som står for "Linear probability model". LPM bruges da den afhængige variabel "participation" er en binær variabel, hvorfor den kun kan tage værdierne 0 og 1. Hertil betyder 1 deltagelse og 0 betyder ikke-deltagelse. 
Sandsynligheden for "succes" ($participation = 1$) er givet ved middelværdien for ($E(y)$). Dermed kan modellen opstilles som $$P(y = 1|x) = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_3 + \beta_4 x_4 + \beta_5 x_5 + \beta_6 x_6 + \beta_7 x_7 + u$$. Hertil er den estimeret model givet ved $$\hat{y} = \hat{\beta_0} + \hat{\beta_1} x_1 + \hat {\beta_2} x_2 + \hat{\beta_3} x_3 + \hat{\beta_4} x_4 + \hat{\beta_5} x_5 + \hat{\beta_6} x_6 + \hat{\beta_7} x_7 + u$$. Her angiver de forskellige $\beta$'er ændringen i sandsynligheden for "succes" (under antagel) 
MANGLER FORKLARING AF LPM + SVAGHEDER
U ER PER DEFINITION HETEROSKEDASTIC - DER BRUGES ROBUST SE
```{r}
model_ols = lm(participation ~ income + age + agesq + educ + youngkids + oldkids + foreign, data = data)
robust_ols = coeftest(model_ols, vcov = vcovHC(model_ols, type = "HC0"))
...
$$H_0: \beta_4 = 0$$
$$H_1: \beta_4 \neq 0$$
T-scoren beregnes ud fra den estimerede $\beta$ samt den tilhørende standardafvigelse. Dette kan gøres, da nulhypotesen er, at den faktiske værdi er nul, hvorfor dette led ikke indgår i formlen.
...
### (b) - Test om den partielle effekt af uddannelse er forskellig fra nul.
Her benyttes igen en t-test som i forrige opgave. Igen opstilles følgende hypoteser.
$$H_0: \beta_4 = 0$$
$$H_1: \beta_4 \neq 0$$
```{r}
...
```
Da t-værdien for både logit- og probit-regressionen er under den kritiske værdi ved et signifikansniveau på 5\% forkastes $H_0$, hvorfor estimatet i begge tilfælde er statistisk signifikant.
```{r}
...
```
Her ses igen en højere p-værdi end det fastsatte signifikansniveau på 5\%, hvilket ligeledes betyder, at $H_0$ kan forkastes. Først ved et signifikansniveau over p-værdien vil $H_0$ ikke kunne forkastes 
