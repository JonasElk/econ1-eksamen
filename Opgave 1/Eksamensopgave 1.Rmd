...
Special udgaven af White-testen minder i høj grad om den ovenstående metode, men er langt mere simpel. Denne er givet ved formelen: 
...
Robuste standartfejl er en metode som kan korrigere standartfejlene i en regressionsmodel, hvor der tages højde for heteroskedasticitet. Metoden kan sikre at inferens er korrekt, i situationer hvor antagelsen om homoskedasticitet er urelaistisk. 
```{r}
...
T-fordelingen er også kendt som "student's t-fordeling", og er en sandsynlighedsfordeling. Den bruges ofte til at estimere en populaitons middelværdi, ved en lav stikprøvestørrelse, og hvor populationens standartafvigelse er ukendt. T-fordelingen ligner en normalfordeling, men den har "fatter tails", hvilket belyser, at der findes større sandsynlighed for ekstremværdier. 
...
$$SSR=\sum_\limits{i=n}^{n}(\hat{u}_i)^2 $$
...
For at beregne F-statistic, kan der gøres brug af formlen for f-test, som er beskrevet ovenstående.
...
Jf. ovenstående er F-værdien 22.6445, hvilket giver en lav p-værdi, hvorfor nulhypotesen, hvor $\beta_3$ og $\beta_4$ er lig med nul, afvises. Derfor vil mindst et af de to estimater have relevans for modellen.

## Opgave 7 - Estimer modellen vha. FGLS og kommenter på resultaterne

FGLS er en forkortelse for Feasible Generalised Least Square. Dette er en metode, som bruges til at estimere parameterne i en lineære regressionsmodel, når der findes heteroskedasticitet. Fra tidligere vides det, at OLS-estimaterne er BLUE under forudsætningerne om, at fejledende har konstant varians (homoskedasticitet) og der ikke findes seriekorrelation. Holder disse forudsætninger ikke, bliver OLS estimaterne inefficiente og standartfejlene upålidelige. Det er i disse tilfælde, at FGLS-metoden spiller en rolle. Formålet med denne metide er nemlig at transformere modellen, så fejledene i den transformeret model opfylder kravene for OLS. 
...
summary(model2fgls)
```
Det ses hvordan alle koefficienter er statistisk signifikante på 0,1\%. Desuden har modellen en høj forklaringsgrad på $R^2 = 0,73$. Alle estimaterne har relativt høje t-værdier, hvilket giver en lav p-værdi og dermed det høje signifikansniveau.

```{r}
screenreg(list(OLS = model1, FGLS = model2fgls), digits = 4)
```
I ovenstående model ses det, at FGLS er signifikant på et højere niveau en OLS - dette skyldes en lavere standartafvigelse. Der ses også en mindre forskel i værdien for $R^2$. 
Desuden findes der en mindre afvigelse på 1 procentpoint ved "education", hvor denne er højest ved OLS. Der ses ligeledes en mindre afvigelse for "male" på 2 procentpoint - denne er ligeledes højest ved OLS. 
